unit Artifacts_Operating_System;
...
const
  ...
  CATEGORY_NAME             = 'Windows Operating System';
  CHAR_LENGTH               = 80;
  COLON                     = ':';
  COMMA                     = ',';
  CR                        = #13#10;
  ...
  PROGRAM_NAME              = 'Windows Operating System';
  RPAD_VALUE                = 55;
  RUNNING                   = '...';
  SCRIPT_DESCRIPTION        = 'Extract Windows Operating System Artifacts';
  SCRIPT_NAME               = 'Operating_System.pas';
...
const { column setup }
  Array_Items_1: TSQL_Table_array = ( // Office 2016 BackstageInAppNavCache
  ...
  Array_Items_2: TSQL_Table_array = ( // Prefetch
  ...
  Array_Items_3: TSQL_Table_array = ( // Program Compatibility Assistant DB (Win11 - PcaGeneralDb0.txt)
  ...
  Array_Items_4: TSQL_Table_array = ( // Program Compatibility Assistant Dic (Win11 - PcaAppLaunchDic.txt)
  ...
  Array_Items_5: TSQL_Table_array = ( // Windows Wifi
  ...
  Array_Items_6: TSQL_Table_array = ( // Windows 10 TimeLine (Activity table)
  ...
  Array_Items_7: TSQL_Table_array = ( // Windows 10 TimeLine (ActivityOperation table) https://docs.microsoft.com/en-us/graph/api/resources/projectrome-historyitem?view=graph-rest-1.0
...
const
  lFileItems: array [1 .. NUMBEROFSEARCHITEMS] of TSQL_FileSearch = (
    ...
    ( // ~1~ Office BackStage
    fi_Process_As: PROCESS_AS_PLIST; fi_Name_Program: 'Office'; // noslz
    fi_Name_Program_Type: 'Backstage'; // noslz
    fi_Icon_Category: 1061; fi_Icon_Program: 1193; fi_Icon_OS: ICON_WINDOWS; fi_Regex_Search: '\.json$'; // noslz
    fi_Signature_Parent: 'JSON'; // noslz
    fi_RootNodeName: 'JSON Tree'; // noslz
    fi_Reference_Info: ''),

    ( // ~2~ Prefetch
    fi_Process_As: PROCESS_AS_PLIST; fi_Name_Program: 'Prefetch'; fi_Name_Program_Type: 'Windows'; fi_Icon_Category: 1061; fi_Icon_Program: 36; fi_Icon_OS: ICON_WINDOWS; fi_Regex_Search: '\.pf$'; // noslz
    fi_Signature_Parent: 'Prefetch'; // noslz
    fi_Signature_Sub: ''; fi_RootNodeName: 'PF Data'; // noslz
    fi_NodeByName: ''; fi_Reference_Info: ''),

    ( // ~3~ Application Execution PcaGeneralDb0.txt
    fi_Process_As: PROCESS_AS_TEXT; fi_Process_ID: 'PcaGeneralDb0'; // noslz
    fi_Name_Program: 'Program Compatibility'; // noslz
    fi_Name_Program_Type: 'Assistant DB'; // noslz
    fi_Icon_Category: 1061; fi_Icon_Program: 32; fi_Icon_OS: ICON_WINDOWS; fi_Regex_Search: 'PcaGeneralDb0\.txt'; // noslz
    fi_RootNodeName: 'JSON Tree'; // noslz
    fi_Reference_Info: ''),

    ( // ~4~ Application Execution PcaAppLaunchDic.txt
    fi_Process_As: PROCESS_AS_TEXT; fi_Process_ID: 'PcaAppLaunchDict'; // noslz
    fi_Name_Program: 'Program Compatibility'; // noslz
    fi_Name_Program_Type: 'Assistant Dic'; // noslz
    fi_Icon_Category: 1061; fi_Icon_Program: 32; fi_Icon_OS: ICON_WINDOWS; fi_Regex_Search: 'PcaAppLaunchDic\.txt'; // noslz
    fi_Signature_Parent: 'Text'; // noslz
    fi_RootNodeName: 'Text Documents'; // noslz
    fi_Reference_Info: ''),

    ( // ~5~ Wifi
    fi_Process_As: PROCESS_AS_PLIST; fi_Name_Program: 'Wifi'; fi_Name_Program_Type: 'Windows'; fi_Icon_Category: 1061; fi_Icon_Program: ICON_WIFI; fi_Icon_OS: ICON_WINDOWS; fi_Regex_Search: '\\Microsoft\\Wlansvc\\Profiles\\Interfaces\\{';
    // noslz
    fi_Signature_Parent: 'XML'; // noslz
    fi_Signature_Sub: ''; fi_RootNodeName: 'XML'; // noslz
    fi_NodeByName: ''; fi_Test_Data: 'IACIS'),

    ( // ~6~ Windows 10 Timeline - Activity
    fi_Process_As: PROCESS_AS_PLIST; fi_Name_Program: 'Windows 10'; fi_Name_Program_Type: 'Timeline (Activity)'; fi_Icon_Category: 1061; fi_Icon_Program: 1145; fi_Icon_OS: ICON_WINDOWS; fi_Regex_Search: 'ActivitiesCache\.db'; // noslz
    fi_Signature_Parent: RS_SIG_SQLITE; fi_Signature_Sub: ''; fi_SQLTables_Required: ['ACTIVITY']; // noslz
    fi_SQLStatement: 'SELECT * FROM ACTIVITY'), // noslz

    ( // ~7~ Windows 10 Timeline - ActivityOperation
    fi_Process_As: PROCESS_AS_PLIST; fi_Name_Program: 'Windows 10'; fi_Name_Program_Type: 'Timeline (ActivityOperation)'; fi_Icon_Category: 1061; fi_Icon_Program: 1145; fi_Icon_OS: ICON_WINDOWS; fi_Regex_Search: 'ActivitiesCache\.db';
    // noslz
    fi_Signature_Parent: RS_SIG_SQLITE; fi_Signature_Sub: ''; fi_SQLTables_Required: ['ACTIVITYOPERATION', 'APPSETTINGS']; // noslz
    fi_SQLStatement: 'SELECT * FROM ACTIVITYOPERATION')); // noslz
...
var
  ...
  DNT_sql_col: string;
  FilesNode: TPropertyNode;
  FilesNodeList: TObjectList;
  FilexList: TObjectList;
  FilexNode: TPropertyNode;
  i, g: integer;
  Item: PSQL_FileSearch;
  ...
  procedure PcaAppLaunchDict(aEntry: TEntry);
  var
    aStringList: TStringList;
    dte_var: string;
    i: integer;
    LineList: TStringList;
    PcaEntryReader: TEntryReader;
    pth_str: string;
  begin
    PcaEntryReader := TEntryReader.Create;
    ...
    if assigned(aEntry) and PcaEntryReader.OpenData(aEntry) and (PcaEntryReader.Size > 0) and Progress.isRunning then
      try
        aStringList.LoadFromStream(PcaEntryReader); // .AsPrintableChar(PcaEntryReader.Size);
        if (aStringList.Count > 0) then
        begin
          for i := 0 to aStringList.Count - 1 do
          begin
            NullTheArray;
            LineList.DelimitedText := aStringList[i];
            if LineList.Count = 2 then
            begin
              pth_str := LineList[0];
              variant_Array[2] := pth_str;
              dte_var := LineList[1];
              setlength(dte_var, 19);
              try
                variant_Array[1] := VarToDateTime(dte_var);
              except
                Progress.Log('Error converting DateTime: ' + Item^.fi_Process_ID); // noslz
              end;
            end;
            AddToModule;
          end;
        ...
        FreeAndNil(LineList);
        FreeAndNil(PcaEntryReader);
      end;
  end;

  procedure PcaGeneralDb0(aEntry: TEntry); { https://aboutdfir.com/new-windows-11-pro-22h2-evidence-of-execution-artifact/ }
  var
    aStringList: TStringList;
    dte_var: string;
    i: integer;
  ...
  begin
    ...
    if assigned(aEntry) and PcaEntryReader.OpenData(aEntry) and (PcaEntryReader.Size > 0) and Progress.isRunning then
      try
        aStringList.Text := PcaEntryReader.AsNullWideString;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              variant_Array[2] := LineList[1];
              variant_Array[3] := LineList[2];
              variant_Array[4] := LineList[3];
              variant_Array[5] := LineList[4];
              variant_Array[6] := LineList[5];
              variant_Array[7] := LineList[6];
              variant_Array[8] := LineList[7];
            end;
            AddToModule;
          end;
        end;
        ...
        try
          ...
          begin
            ...
            begin
              ...
              begin
                if Item^.fi_Process_ID = 'PcaAppLaunchDict' then
                  PcaAppLaunchDict(aArtifactEntry); // noslz
                if Item^.fi_Process_ID = 'PcaGeneralDb0' then
                  PcaGeneralDb0(aArtifactEntry); // noslz
              end;
                ...
                try
                  ...
                  begin
                    ...
                    begin
                      ...
                      // ==========================================================
                      // Locate Node by Name Method
                      // ==========================================================
                      begin
                        // BackStage
                        if Item^.fi_Name_Program_Type = 'Backstage' then // noslz
                        begin
                          if not Progress.isRunning then
                            break;

                          // Go to the Files [] node
                          FilesNode := TPropertyNode(aRootProperty.GetFirstNodeByName('Files []'));
                          if assigned(FilesNode) then
                          begin
                            FilesNodeList := FilesNode.PropChildList;
                            if assigned(FilesNodeList) then
                            begin
                              for x := 0 to FilesNodeList.Count - 1 do
                              begin
                                aNode := TPropertyNode(FilesNodeList.items[x]);
                                if assigned(aNode) then
                                begin
                                  FilexList := aNode.PropChildList;
                                  if assigned(FilexList) then
                                  begin
                                    for y := 0 to FilexList.Count - 1 do
                                    begin
                                      FilexNode := TPropertyNode(FilexList.items[y]);
                                      if assigned(FilexNode) then
                                      begin
                                        for g := 1 to ColCount do
                                        begin
                                        DNT_sql_col := aItems[g].sql_col;
                                        Delete(DNT_sql_col, 1, 4);
                                        if (UpperCase(FilexNode.PropName) = UpperCase(DNT_sql_col)) then
                                        begin
                                        // String
                                        if aItems[g].col_type = ftString then
                                        variant_Array[g] := FilexNode.PropDisplayValue
                                        else
                                        // FileDate
                                        if (aItems[g].col_type = ftDateTime) and (aItems[g].convert_as = 'FTME') and // noslz
                                        (UpperCase(FilexNode.PropName) = 'LASTMODIFIED') then // noslz
                                        begin
                                        if RegexMatch(FilexNode.PropDisplayValue, '\d{18}', False) then
                                        try
                                        tmp_int64 := StrToInt64(FilexNode.PropDisplayValue);
                                        tmp_dt := FileTimeToDateTime(tmp_int64);
                        ...
                        begin
                          ...
                          begin
                            ...
                            begin
                              ...
                              begin
                                test_str := '';
                                test_str := (Node1.PropDisplayValue);
                                test_str := StringReplace(test_str, 'Mon' + SPACE, '', [rfReplaceAll]);
                                test_str := StringReplace(test_str, 'Tue' + SPACE, '', [rfReplaceAll]);
                                test_str := StringReplace(test_str, 'Wed' + SPACE, '', [rfReplaceAll]);
                                test_str := StringReplace(test_str, 'Thu' + SPACE, '', [rfReplaceAll]);
                                test_str := StringReplace(test_str, 'Fri' + SPACE, '', [rfReplaceAll]);
                                test_str := StringReplace(test_str, 'Sat' + SPACE, '', [rfReplaceAll]);
                                test_str := StringReplace(test_str, 'Sun' + SPACE, '', [rfReplaceAll]);
                                test_str := StringReplace(test_str, 'Monday' + SPACE, '', [rfReplaceAll]);
                                test_str := StringReplace(test_str, 'Tuesday' + SPACE, '', [rfReplaceAll]);
                                test_str := StringReplace(test_str, 'Wednesday' + SPACE, '', [rfReplaceAll]);
                                test_str := StringReplace(test_str, 'Thursday' + SPACE, '', [rfReplaceAll]);
                                test_str := StringReplace(test_str, 'Friday' + SPACE, '', [rfReplaceAll]);
                                test_str := StringReplace(test_str, 'Saturday' + SPACE, '', [rfReplaceAll]);
                                test_str := StringReplace(test_str, 'Sunday' + SPACE, '', [rfReplaceAll]);
                                try
                                  variant_Array[g] := VarToDateTime(test_str);
                                except
