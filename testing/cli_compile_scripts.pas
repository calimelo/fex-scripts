...
begin
  ...
  try
    ...
    begin
      ...
      Progress.Log(astr);
      if DirectoryExists(astr + 'Scripts') then
      begin
        astr := astr + 'Scripts';
        break;
      end;
    ...
    if (not RegexMatch(astr, '\\Scripts\\?$', False)) then
    begin
      ConsoleLog('A - Could not find scripts start folder: ' + StartDir + '.' + SPACE + TSWT);
      ConsoleLog(StringOfChar('-', CHAR_LENGTH));
      ConsoleLog('Finished.');
      Exit;
    end;
    ...
    begin
      ConsoleLog('B - Could not find scripts start folder: ' + StartDir + '.' + SPACE + TSWT);
      ConsoleLog(StringOfChar('-', CHAR_LENGTH));
      ConsoleLog('Finished.');
      Exit;
    end;
    ...
    try
      ConsoleLog(StringOfChar('-', CHAR_LENGTH));
      ConsoleLog(RPad('Scripts StartDir:', RPAD_VALUE) + StartDir);
